...
/**
 * Check if we're on a Bloglo admin page.
 *
 * @since 1.0.0
 ...
 * @return boolean
 */
function bloglo_is_admin_page( $base = false, $slug = 'bloglo' ) {
...
function bloglo_print_notice( $args ) {
	...
	<div id="<?php echo esc_attr( $args['message_id'] ); ?>" class="notice bloglo-notice notice-<?php echo esc_attr( $args['type'] ); ?><?php echo esc_attr( $bloglo_is_dismissible ); ?>">
		<p><?php echo ( wp_kses( $args['message'], bloglo_get_allowed_html_tags() ) ); ?></p>

		<?php
		if ( $args['action_link'] && $args['action_text'] ) {
			?>
			<p class="bloglo-notice-action">
				<a href="<?php echo esc_url( $args['action_link'] ); ?>" class="bloglo-btn primary button button-primary" role="button"><?php echo esc_html( $args['action_text'] ); ?></a>

				<?php
				if ( $args['dismiss_text'] ) {
					?>
					<a href="#" class="bloglo-btn secondary button button-secondary bloglo-notice-dismiss" role="button"><?php echo esc_html( $args['dismiss_text'] ); ?></a>
					<?php
				}
				?>
			</p><!-- END .bloglo-notice-action -->
			<?php
		}
		?>
	</div>
	...
	<script type="text/javascript">
		jQuery( document ).ready( function ( $ ) {
			...
			$el.on( 'click', '.notice-dismiss, .bloglo-notice-dismiss', function ( event ) {

				var expires = "<?php echo esc_attr( $args['expires'] ); ?>";
				var nonce = "<?php echo esc_attr( wp_create_nonce( 'bloglo_dismiss_notice' ) ); ?>";

				$.post( ajaxurl, {
					action: 		'bloglo_dismiss_notice',
					msgid: 			msgid,
					expires: 		expires,
					_ajax_nonce: 	nonce,
				} );
 ...
 * @return boolean
 */
function bloglo_is_notice_dismissed( $id ) {

	if ( false !== get_transient( 'bloglo_notice_' . $id ) ) {
		return true;
	}
 ...
 * @return void
 */
function bloglo_dismiss_notice() {

	check_ajax_referer( 'bloglo_dismiss_notice' );
	...
	$message              = (array) get_transient( 'bloglo_notice_' . $message_id );
	$message['time']      = time();
	$message['dismissed'] = true;

	set_transient( 'bloglo_notice_' . $message_id, $message, $expires );
	die;
}
add_action( 'wp_ajax_bloglo_dismiss_notice', 'bloglo_dismiss_notice' );
 ...
 * @return void
 */
function bloglo_print_admin_icon( $icon = 'info', $tooltip = '', $tooltip_position = 'right-tooltip' ) {
	...
	if ( '' !== $tooltip ) {
		$tooltip = '<span class="bloglo-tooltip ' . esc_attr( $tooltip_position ) . '">' . esc_html( $tooltip ) . '</span>';
	}

	if ( 'warning' === $icon ) {
		echo '<i class="bloglo-warning-icon">' . $svg_icon . $tooltip . '</i>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	} elseif ( 'info' === $icon ) {
		echo '<i class="bloglo-info-icon">' . $svg_icon . $tooltip . '</i>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
...
function bloglo_help_link( $args = array(), $echo = true ) {
	...
	$output = sprintf(
		'<a href="%1$s" rel="nofollow" target="_blank" class="%2$s"><span class="bloglo-help-icon">%4$s</span>%3$s</a>',
		esc_url( $args['link'] ),
		esc_attr( $class ),
