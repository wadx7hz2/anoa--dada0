...
if ( ! class_exists( 'Bloglo_Meta_Boxes' ) ) :

	/**
	 * Meta Boxes Class.
	 */
	class Bloglo_Meta_Boxes {
		...
		public function metabox_config() {
			...
			// Sidebar position.
			$layout_settings['bloglo_sidebar_position'] = array(
				'id'      => 'bloglo_sidebar_position',
				'name'    => esc_html__( 'Sidebar', 'bloglo' ),
				'type'    => 'select',
				'default' => 'default',
				'choices' => array(
					''              => esc_html__( 'Default (from Customizer)', 'bloglo' ),
			...
			// Content layout.
			$layout_settings['bloglo_content_layout'] = array(
				'id'      => 'bloglo_content_layout',
				'name'    => esc_html__( 'Content Layout', 'bloglo' ),
				'type'    => 'select',
				'default' => 'default',
				'choices' => array(
					''             => esc_html__( 'Default (from Customizer)', 'bloglo' ),
					...
					'boxed'        => esc_html__( 'Boxed', 'bloglo' ),
				),
			);

			// Transparent header.
			$layout_settings['bloglo_transparent_header'] = array(
				'id'      => 'bloglo_transparent_header',
				'name'    => esc_html__( 'Transparent Header', 'bloglo' ),
				'type'    => 'select',
				'default' => '',
				'choices' => array(
					''        => esc_html__( 'Default (from Customizer)', 'bloglo' ),
					'enable'  => esc_html__( 'Enable', 'bloglo' ),
					'disable' => esc_html__( 'Disable', 'bloglo' ),
				),
			);

			// Start checkbox group.
			$layout_settings['bloglo_page_sections'] = array(
				'id'     => 'bloglo_page_sections',
				'type'   => 'group',
				'fields' => array(),
			);

			// Disable Top Bar.
			if ( bloglo_option( 'top_bar_enable' ) ) {
				$layout_settings['bloglo_page_sections']['fields']['bloglo_disable_topbar'] = array(
					'id'      => 'bloglo_disable_topbar',
					'name'    => esc_html__( 'Disable Top Bar', 'bloglo' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			// Disable Header.
			$layout_settings['bloglo_page_sections']['fields']['bloglo_disable_header'] = array(
				'id'      => 'bloglo_disable_header',
				'name'    => esc_html__( 'Disable Main Header', 'bloglo' ),
				'default' => false,
				'type'    => 'checkbox',
			);
			...
			if ( bloglo_option( 'page_header_enable' ) ) {
				$layout_settings['bloglo_page_sections']['fields']['bloglo_disable_page_title'] = array(
					'id'      => 'bloglo_disable_page_title',
					'name'    => esc_html__( 'Disable Page Title', 'bloglo' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			if ( bloglo_option( 'page_header_enable' ) && bloglo_option( 'breadcrumbs_enable' ) && ! bloglo_is_section_disabled( bloglo_option( 'breadcrumbs_hide_on' ), get_the_ID() ) ) {
				$layout_settings['bloglo_page_sections']['fields']['bloglo_disable_breadcrumbs'] = array(
					'id'      => 'bloglo_disable_breadcrumbs',
					'name'    => esc_html__( 'Disable Breadcrumbs', 'bloglo' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			$layout_settings['bloglo_page_sections']['fields']['bloglo_disable_thumbnail'] = array(
				'id'      => 'bloglo_disable_thumbnail',
				'name'    => esc_html__( 'Disable Featured Image', 'bloglo' ),
				'default' => false,
				'type'    => 'checkbox',
			);

			// Disable Prefooter.
			if ( bloglo_option( 'enable_pre_footer_cta' ) && ! bloglo_is_section_disabled( bloglo_option( 'pre_footer_cta_hide_on' ), get_the_ID() ) ) {
				$layout_settings['bloglo_page_sections']['fields']['bloglo_disable_prefooter_cta'] = array(
					'id'      => 'bloglo_disable_prefooter_cta',
					'name'    => esc_html__( 'Disable Pre Footer CTA', 'bloglo' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			if ( bloglo_option( 'enable_footer' ) ) {
				$layout_settings['bloglo_page_sections']['fields']['bloglo_disable_footer'] = array(
					'id'      => 'bloglo_disable_footer',
					'name'    => esc_html__( 'Disable Main Footer', 'bloglo' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			if ( bloglo_option( 'enable_copyright' ) ) {
				$layout_settings['bloglo_page_sections']['fields']['bloglo_disable_copyright'] = array(
					'id'      => 'bloglo_disable_copyright',
					'name'    => esc_html__( 'Disable Copyright Bar', 'bloglo' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}

			// Disable Post Card Border.
			if ( bloglo_option( 'blog_card_border' ) ) {
				$layout_settings['bloglo_page_sections']['fields']['bloglo_disable_blog_card_border'] = array(
					'id'      => 'bloglo_disable_blog_card_border',
					'name'    => esc_html__( 'Disable Post Card Border', 'bloglo' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			// Set metabox options.
			$this->metabox = array(
				'bloglo-theme-page-options' => array(
					'id'         => 'bloglo-theme-page-options',
					'post_type'  => $this->get_post_types(),
					'capability' => 'edit_posts',
					'label'      => esc_html__( 'Theme Page Options', 'bloglo' ),
					'context'    => 'side',
					'priority'   => 'core',
			...
			$this->metabox = apply_filters( 'bloglo_metabox_options', $this->metabox );
		}
		...
		public function render( $post, $metabox ) {
			...
			foreach ( $metabox['args']['settings'] as $setting ) {
				...
				echo '<div class="bloglo-meta-box-field">';
		...
		public function render_field_checkbox( $post, $setting ) {
			...
			?>
			<div class="components-panel__row bloglo-checkbox-row">
				<div class="components-base-control">
					<div class="components-base-control__field">
						<span class="components-checkbox-control__input-container bloglo-checkbox-wrapper">
							...
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" role="img" class="bloglo-checked-icon components-checkbox-control__checked" aria-hidden="true" focusable="false"><path d="M9 18.6L3.5 13l1-1L9 16.4l9.5-9.9 1 1z"></path></svg>
						</span>
		...
		public function load_assets( $hook ) {
			...
			if ( 'post-new.php' === $hook || 'post.php' === $hook ) {
				...
				if ( in_array( $post_type, $post_types, true ) ) {
					...
					wp_enqueue_style(
						'bloglo-metabox-style',
						BLOGLO_THEME_URI . '/inc/admin/assets/css/bloglo-meta-boxes' . $bloglo_suffix . '.css',
						false,
...
/**
 * The function which returns the one Bloglo Meta Boxes instance.
 *
 ...
 * @return object
 */
function bloglo_meta_boxes() {
	return Bloglo_Meta_Boxes::get_instance();
}
