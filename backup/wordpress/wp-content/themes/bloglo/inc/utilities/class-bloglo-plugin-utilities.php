...
class Bloglo_Plugin_Utilities {
	...
	/**
	 * Main Bloglo Plugin Utilities Instance.
	 *
	 * @since 1.0.0
	 * @return Bloglo_Plugin_Utilities
	 */
	public static function instance() {

		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Bloglo_Plugin_Utilities ) ) {
	...
	public function activate_plugin( $plugin ) {
		...
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return new WP_Error(
				...
				esc_html__( 'Current user can\'t activate plugins', 'bloglo' )
			);
		}
		...
		if ( empty( $plugin ) ) {
			return new WP_Error(
				'plugin_activation_failed',
				esc_html__( 'Missing plugin data.', 'bloglo' )
			);
		}
		...
		if ( empty( $plugin_data ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					esc_html__( 'Plugin %s is not installed.', 'bloglo' ),
					$plugin
				)
			);
		}
		...
		do_action( 'bloglo_plugin_activated_' . $plugin );
	}
	...
	public function deactivate_plugin( $plugin ) {
		...
		if ( empty( $plugin_data ) ) {
			return new WP_Error(
				...
				sprintf(
					// translators: %s is plugin name.
					esc_html__( 'Plugin %s is not active.', 'bloglo' ),
					$plugin
				)
			);
		}
		...
		do_action( 'bloglo_plugin_deactivated_' . $plugin );
	}
	...
	public function get_recommended_plugins() {
		...
		$plugins = array(
			...
			'name'  => 'Hester Core',
			'slug'  => 'hester-core',
			'desc'  => 'The Hester Core plugin adds extra functionality to Bloglo theme, such as Demo Library, widgets, custom blocks and more.',
		...
		return apply_filters( 'bloglo_recommended_plugins', $plugins );
	}
 ...
 *
 * Example: <?php $bloglo_plugin_utilities = bloglo_plugin_utilities(); ?>
 *
 * @since 1.0.0
 * @return object
 */
function bloglo_plugin_utilities() {
	return Bloglo_Plugin_Utilities::instance();
}
