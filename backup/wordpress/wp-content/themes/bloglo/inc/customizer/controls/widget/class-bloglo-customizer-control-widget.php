<?php
/**
 * Bloglo Customizer Widget control class.
 *
 * @package     Bloglo
...
if ( ! class_exists( 'Bloglo_Customizer_Control_Widget' ) ) :
	/**
	 * Bloglo Customizer custom select control class.
	 */
	class Bloglo_Customizer_Control_Widget extends Bloglo_Customizer_Control {
		...
		public function to_json() {
			...
			if ( is_array( $this->widgets ) && ! empty( $this->widgets ) ) {
				...
				foreach ( $this->widgets as $widget_id => $args ) {
					...
					ob_start();
					?>
					<div data-widget-id="<?php echo esc_attr( $instance->id_base ); ?>" class="bloglo-widget-tpl-<?php echo esc_attr( $instance->id_base ); ?> bloglo-widget" data-max-uses=<?php echo esc_attr( $max_uses ); ?>>
		...
		protected function content_template() {
			?>
			<div class="bloglo-control-wrapper bloglo-widget-wrapper">

				<div class="bloglo-widget-container">

					<# _.each( data.value, function( template, i ) { #>
						{{{ template }}}
					<# }); #>

				</div><!-- END .bloglo-widget-container -->

				<div class="bloglo-add-widget-wrap">			
					<button type="button" class="button bloglo-add-widget" data-location-title="{{{ data.label }}}" data-control="{{{ data.id }}}"><?php esc_html_e( 'Add Widget', 'bloglo' ); ?></button>
				</div>

				<div class="bloglo-widget-tmpls">
